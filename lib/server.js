...
*/
    ...
    this.onStateChange = (id, state) => {
        ...
        if (server && state && !state.ack) {
            ...
            if (stateId.match(/PWM_/)) {
		    stateId = stateId.slice(4);
	    }
    ...
    function checkData(client, topic, prefix, data, unit, path) {
	    ...
	    if(attr.match(/.*PWM.*/i)) {
		    adapter.log.debug(`PWM!!! topic: ${topic} prefix: ${prefix} attr: ${attr} data[attr]: ${JSON.stringify(data[attr], null, 2)}`);
	    }
		...
		adapter.log.debug(`topic: ${topic} attr: ${attr} replaceAttr: ${replaceAttr} obj.data.common: ${JSON.stringify(obj.data.common, null, 2)}`);
    ...
    (function _constructor(config) {
        ...
        server.on('connection', stream => {
		    ...
		    client._map = client._map || {}
		    for (let i = 1; i <=8; i++) {
			    client._map[`PWM_PWM${i}`] = `cmnd/${client.id}/PWM${i}`;
		    }
